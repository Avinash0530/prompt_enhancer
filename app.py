...
AudioSegment.ffmpeg = r'C:\ffmpeg-2024-07-24-git-896c22ef00-full_build\bin\ffmpeg.exe'  # Adjust this path to where FFmpeg is installed
...
DEFAULT_AUDIO_PATH = r'C:\s to t\recorded_audio.wav'
...
def recognize_from_audio_file(audio_file_path, language_code):
    ...
    try:
        ...
        print(f"Attempting to recognize speech with language code: {language_code}")  # Debugging
...
def convert_webm_to_wav(webm_path, wav_path):
    ffmpeg_path = r'C:\ffmpeg-2024-07-24-git-896c22ef00-full_build\bin\ffmpeg.exe'  # Replace with the actual path to ffmpeg.exe
    command = [
        ffmpeg_path, '-y', '-i', webm_path, '-acodec', 'pcm_s16le', '-ar', '16000', '-ac', '1', wav_path
    ]
    try:
        subprocess.run(command, check=True)
        print(f"Conversion successful: {webm_path} to {wav_path}")  # Debugging
    ...
    if input_type == 'microphone':
        ...
        if language_code:
            ...
            if audio_file:
                ...
                # Debugging: Check if the WAV file exists
                if os.path.exists(wav_path):
                    print(f"WAV file saved at: {wav_path}")
                else:
                    print(f"Failed to save WAV file at: {wav_path}")
                    result['error'] = 'Failed to convert audio file.'
